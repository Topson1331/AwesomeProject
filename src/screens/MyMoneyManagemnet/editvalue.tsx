...
export default function ({
  ...
  const [the_date, setDate] = useState<string>(route.params.the_date);
  const [amount, setAmount] = useState<string>(route.params.amount);
  const [typeValue, setTypeValue] = useState<string>(route.params.typeValue);
  const [accountvalue, setAccountValue] = useState<string>(
    route.params.accountvalue
  );
  ...
  async function editValue() {
    try {
      ...
      const docRef = await updateDoc(doc(db, "Add_value", route.params.key), {
        typeValue: typeValue,
      ...
      Alert.alert("Edit successfully. Document written with ID: ");
    } catch (error: any) {
      setLoading(false);
